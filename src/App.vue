<template>
  <div class="container" :class="{ disconnected: !connected }">
    ...
    <svg viewBox="0 0 264.58333 264.58333">
      <Branch v-for="branch in flatReefStateMap" :key="branch.location" :points="getBranchPath(branch.location)"
        @click="toggleBranch(branch.face, branch.level, branch.side)" :status="branch.state" />
      />
      <Algae v-for="face in faces" :key="face" :points="getAlgaePath(face)" @click="toggleAlgae(face)" :present="algaeStates[face]" />
...
import { NT4_Client } from './NT/NT4';
import Branch from './components/Branch.vue';
import Algae from './components/Algae.vue';
...
const getBranchPath = (path: string): string => {
  return (svg.svg.g[1].path?.find(p => p['_inkscape:label'] === path)?._d)!;
};
const getAlgaePath = (face: Face): string => {
  return (svg.svg.g[1].path?.find(p => p['_inkscape:label'] === `alg${face}`.toLowerCase())?._d)!;
};
...
const branchStates = ['FREE', 'OCCUPIED', 'DISABLED'] as const;
...
type ReefState = Record<Face, Record<Level, Record<Side, BranchState>>>;
...
const emptyReef: ReefState = faces.reduce((reefAcc, face) => {
  reefAcc[face] = levels.reduce((levelAcc, level) => {
    levelAcc[level] = sides.reduce((sideAcc, side) => {
      sideAcc[side] = 'OCCUPIED'; // default value
      return sideAcc;
...
const flatReefStateMap = computed(() => Object.entries(reefState).flatMap(([face, levels]): { location: string, state: BranchState, face: Face, level: Level, side: Side }[] => {
...
const nt = new NT4_Client('localhost', "Dashboard", (topic) => { }, (topic) => { },
  (topic, timestamp, value) => {
    const x = topic.name.split('Reef/')[1].split('/');
    const face = x[0] as Face;
    if(x[1] === 'Algae') {
      console.log(`${topic.name} Received ${value} for ${face}`);
      algaeStates[face] = value;
    } else {
      const level = x[1] as Level;
      const side = x[2] as Side;
      console.log(`${topic.name} Received ${value} for ${face}-${level}-${side}`);
      reefState[face][level][side] = branchStates[value];
    }
  },
...
const topics = faces.flatMap(face => levels.flatMap(level => sides.map(side => `Reef/${face}/${level}/${side}`))).concat(faces.map(face => `Reef/${face}/Algae`));
nt.subscribe(topics.map(topic => "/Dashboard/robot/" + topic), false);

topics.forEach((topic) => {
  nt.publishTopic("/Dashboard/dashboard/" + topic, topic.split('/').pop() === 'Algae' ? "boolean" : "int");
});
...
const toggleBranch = (face: Face, level: Level, side: Side) => {
  nt.addSample("/Dashboard/dashboard/Reef/" + face + "/" + level + "/" + side, (branchStates.indexOf(reefState[face][level][side]) + 1) % branchStates.length);
};

const toggleAlgae = (face: Face) => {
  nt.addSample("/Dashboard/dashboard/Reef/" + face + "/Algae", !algaeStates[face]);
};
</script>
